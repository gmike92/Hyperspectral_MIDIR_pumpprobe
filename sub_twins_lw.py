...
class SpectrumProcessor:
    ...
    def __init__(self):
        ...
        # Cached burst-center index: computed on first spectrum call, reused thereafter.
    ...
    def reset_center(self):
        """Clear the cached burst-center so it is recomputed on the next spectrum call."""
        self.center_idx = None
    ...
    def apodization(self, data, positions, width=0.2, force_center=None):
        ...
        The burst-center index is cached on the first call (`self.center_idx`)
        and reused for all subsequent calls until the scan is reset.
        """
        if force_center is not None:
            center_idx = force_center
        else:
            if self.center_idx is None:
                self.center_idx = int(np.argmax(data))
    ...
    def max_step_um(self, wl_short_um, samples_per_cycle=5):
        ...
        per optical cycle at the shortest wavelength. samples_per_cycle=2 is
        the Nyquist limit; 5 gives a safe oversampling (default).

            k_max  = reciprocal_cal(1/λ_short)              [1/mm-stage]
            Δx_max = 1 / (samples_per_cycle · k_max)        [mm]

        Returns None if calibration is missing or inputs are invalid.
        """
        if wl_short_um is None or wl_short_um <= 0:
            return None
        if samples_per_cycle is None or samples_per_cycle <= 0:
            return None
        if self.wavelength_cal is None or self.reciprocal_cal is None:
            # Fallback (no birefringence info): λ_short / samples_per_cycle in µm
        ...
        try:
            ...
            if k_max <= 0:
                return None
            dx_mm = 1.0 / (float(samples_per_cycle) * k_max)
            return dx_mm * 1000.0  # µm
        except Exception:
            return None
    ...
    def estimate_resolution_nm(self, scan_range_mm, wl_center_um):
        """Estimate spectral resolution (FWHM, in nm) from stage scan range.

        Uses the calibration's local slope d(reciprocal_cal)/d(1/λ) at λ_c
        to convert the stage-Fourier resolution Δk = 1/L into a wavelength
        resolution that accounts for the TWINS birefringence + wedge geometry.

        Returns None if calibration is missing or inputs are invalid.
        """
        if scan_range_mm is None or scan_range_mm <= 0:
            return None
        if wl_center_um is None or wl_center_um <= 0:
            return None
        if self.wavelength_cal is None or self.reciprocal_cal is None:
            # Fallback: simple λ²/L (ignores birefringence — gives wrong order of magnitude for TWINS)
            return (wl_center_um ** 2 / scan_range_mm) * 1000.0
        try:
            from scipy.interpolate import interp1d
            fn = interp1d(1.0 / self.wavelength_cal, self.reciprocal_cal,
                          kind="linear", fill_value="extrapolate")
            inv_lambda_c = 1.0 / wl_center_um  # 1/µm
            eps = max(inv_lambda_c * 1e-3, 1e-6)
            slope = float((fn(inv_lambda_c + eps) - fn(inv_lambda_c - eps)) / (2 * eps))
            if slope == 0:
                return None
            delta_recip = 1.0 / scan_range_mm                  # 1/mm-stage
            delta_inv_lambda = abs(delta_recip / slope)        # 1/µm
            delta_wl_um = (wl_center_um ** 2) * delta_inv_lambda
            return delta_wl_um * 1000.0                        # nm
        except Exception:
            return None
    ...
    def compute_spectrum(self, wl_start=8.0, wl_stop=14.0,
        ...
        if self.center_idx is None:
            self.center_idx = int(np.argmax(signal))
...
class TwinsWindow(QtWidgets.QWidget):
    ...
    def _setup_ui(self):
        ...
        range_layout.addWidget(self.lbl_step_size, 4, 1)

        range_layout.addWidget(QtWidgets.QLabel("Resolution:"), 5, 0)
        self.lbl_resolution = QtWidgets.QLabel("-- nm")
        self.lbl_resolution.setStyleSheet("font-weight: bold; color: #2196F3;")
        self.lbl_resolution.setToolTip("Spectral resolution at the center of the WL window, computed from the calibration's local birefringence factor.")
        range_layout.addWidget(self.lbl_resolution, 5, 1)

        range_layout.addWidget(QtWidgets.QLabel("Min step (5×):"), 6, 0)
        self.lbl_min_step = QtWidgets.QLabel("-- µm")
        self.lbl_min_step.setStyleSheet("font-weight: bold;")
        self.lbl_min_step.setToolTip("Maximum stage step that yields 5 points per optical cycle at the shortest wavelength (Nyquist is 2; 5× gives margin for short-λ work and calibration uncertainty). Current step must be ≤ this value.")
        range_layout.addWidget(self.lbl_min_step, 6, 1)
        ...
        self.spin_wl_start.valueChanged.connect(self._update_step_display)
        self.spin_wl_stop.valueChanged.connect(self._update_step_display)
    ...
    def _update_step_display(self):
        ...
        # Resolution estimate using the calibration (handles TWINS birefringence)
        scan_range_mm = abs(stop - start)
        wl_center = 0.5 * (self.spin_wl_start.value() + self.spin_wl_stop.value())
        res_nm = self.processor.estimate_resolution_nm(scan_range_mm, wl_center)
        if res_nm is None:
            self.lbl_resolution.setText("-- nm")
        else:
            self.lbl_resolution.setText(f"~{res_nm:.0f} nm @ {wl_center:.1f} µm")

        # 5-points-per-cycle step at the shortest wavelength in the window
        wl_short = self.spin_wl_start.value()
        max_um = self.processor.max_step_um(wl_short, samples_per_cycle=5)
        if max_um is None:
            self.lbl_min_step.setText("-- µm")
            self.lbl_min_step.setStyleSheet("font-weight: bold;")
        else:
            self.lbl_min_step.setText(f"{max_um:.2f} µm @ {wl_short:.1f} µm")
            # Color: red if step exceeds 5×-cycle target, green if OK
            if step_um is not None and step_um > max_um:
                self.lbl_min_step.setStyleSheet("font-weight: bold; color: #f44336;")
            else:
                self.lbl_min_step.setStyleSheet("font-weight: bold; color: #4CAF50;")
    ...
    def _start_scan(self):
        ...
        # New scan → discard any previously cached burst center
    ...
    def _poll_acquire(self):
        ...
        try:
            ...
            if odd_data is not None and even_data is not None:
                # Explicit copy so in-place bg subtraction can't corrupt a buffer shared with LabVIEW
                odd = np.array(odd_data, dtype=float, copy=True)
                even = np.array(even_data, dtype=float, copy=True)
                ...
                img_t = (odd + even) / 2.0
                img_dt = even - odd
                # ΔT/T guard: threshold scales with signal so it works regardless of units (counts, volts, normalized)
                odd_abs_max = float(np.max(np.abs(odd))) if odd.size else 0.0
                dtt_threshold = max(1e-9, odd_abs_max * 1e-3)
                img_dtt = np.divide(even - odd, odd, out=np.zeros_like(odd), where=np.abs(odd) > dtt_threshold) * 100.0
    ...
    def _load_data(self):
        ...
        if filepath:
            try:
                ...
                # Fresh data → forget the cached burst center
                self.processor.reset_center()
