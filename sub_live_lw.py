...
class LiveViewWindow(QtWidgets.QWidget):
    ...
    def _setup_ui(self):
        ...
        self.spin_stage_target.setValue(140.0)
        ...
        def _set_zero_from_stage():
            if self.delay_stage and self.delay_stage.is_connected:
                self.spin_zero.setValue(self.delay_stage.get_position())

        self.btn_set_zero = QtWidgets.QPushButton("Set from current")
        self.btn_set_zero.setToolTip("Set zero to the current stage position")
        self.btn_set_zero.clicked.connect(_set_zero_from_stage)

        zero_layout.addWidget(QtWidgets.QLabel("Zero:"))
        zero_layout.addWidget(self.spin_zero)
        zero_layout.addWidget(self.btn_set_zero)
