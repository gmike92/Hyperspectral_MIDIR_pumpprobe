...
class MainLauncher(QtWidgets.QWidget):
    ...
    def _setup_ui(self):
        ...
        # Save Directory
        # =================================================================
        save_group = QtWidgets.QGroupBox("Save Directory")
        save_layout = QtWidgets.QHBoxLayout(save_group)
        layout.addWidget(save_group)

        self.save_dir_edit = QtWidgets.QLineEdit(SaveConfig().base_dir)
        self.save_dir_edit.setStyleSheet(
            "background-color: #2a2a2a; color: #e0e0e0; border: 1px solid #555; "
            "border-radius: 4px; padding: 4px 8px;"
        )
        self.save_dir_edit.editingFinished.connect(self._on_save_dir_edited)
    ...
    def _browse_save_dir(self):
        path = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Select Save Directory", SaveConfig().base_dir
        )
        if path:
            SaveConfig().set_base_dir(path)
            self.save_dir_edit.setText(SaveConfig().base_dir)
            self._log(f"[OK] Save directory: {SaveConfig().base_dir}")

    def _on_save_dir_edited(self):
